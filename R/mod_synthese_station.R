...
mod_synthese_station_server <- function(id, resumes_listes, stations, regie, indices, acronymes_indices, valeurs_seuils_stations, parametres_eqr, etat_bio, listes_taxo, choix_station, choix_eqb, ordre_taxon, choix_stations){
  moduleServer( id, function(input, output, session){
    ...
    fiche_demande <- reactiveVal(NULL)

    observeEvent(input$btn_afficher_fiche, {
      req(choix_station())

      # Capture la valeur pour usage futur
      code_station <- choix_station()

      # Afficher message de chargement
      output$loading_message <- renderUI({
        div(class = "text-info", "Chargement des fiches en cours...")
      })

      later::later(function() {
        # Utiliser la valeur capturée, pas la fonction réactive
        fiche_demande(code_station)

        later::later(function() {
          output$loading_message <- renderUI(NULL)
        }, delay = 2)

      }, delay = 0.1)
    })
    observeEvent(choix_station(), {
      fiche_demande(NULL)
    })
